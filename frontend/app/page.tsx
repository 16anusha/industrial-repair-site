...
function ChatbotWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div className="w-96 h-[500px] border border-zinc-700 shadow-2xl rounded-xl flex flex-col bg-zinc-900 text-white mb-4 animate-in fade-in slide-in-from-bottom-4">
          <div className="p-3 bg-zinc-800 rounded-t-xl flex justify-between items-center border-b border-zinc-700">
            <span className="font-bold text-sm">SNAPS AI Assistant</span>
            <button 
              onClick={() => setIsOpen(false)} 
              className="text-zinc-400 hover:text-white font-bold px-2"
            >
              ✕
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-zinc-700">
            {messages.length === 0 && (
              <div className="text-zinc-500 text-sm text-center mt-10 font-mono">
          ...
          <form onSubmit={sendMessage} className="p-3 flex gap-2 border-t border-zinc-700 bg-zinc-900 rounded-b-xl">
            <input 
              type="text" 
              value={input}
              onChange={(e) => setInput(e.target.value)}
              className="flex-1 bg-zinc-950 border border-zinc-700 rounded-lg p-2 text-white outline-none text-sm focus:border-red-500 transition-colors"
              placeholder="Ask a technical question..."
            />
            <button type="submit" className="bg-red-600 hover:bg-red-500 px-4 rounded-lg font-bold text-sm transition-colors">Send</button>
          </form>
        </div>
      )}
      ...
      <button
        ...
        className="bg-red-600 hover:bg-red-500 text-white px-5 py-3 rounded-full shadow-[0_0_15px_rgba(220,38,38,0.4)] font-medium flex items-center gap-2 transition-transform hover:scale-105"
      >
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-red-500 selection:text-white relative">
      
      {/* 1. COMPREHENSIVE NAV BAR */}
      <nav className="border-b border-zinc-800 bg-zinc-950/90 backdrop-blur sticky top-0 z-40">
      ...
      {/* 8. OUR INTEGRATED CHATBOT WIDGET */}
      <ChatbotWidget />
